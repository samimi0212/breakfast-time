  ...
  return (
    <div className="min-h-screen bg-background">
      <style>{`
        ...
        .conf-cf1 { animation: cf1 0.7s ease-out 0.3s both; } .conf-cf2 { animation: cf2 0.75s ease-out 0.35s both; }
        .conf-cf3 { animation: cf3 0.65s ease-out 0.28s both; } .conf-cf4 { animation: cf4 0.8s ease-out 0.4s both; }
        .conf-cf5 { animation: cf5 0.7s ease-out 0.32s both; } .conf-cf6 { animation: cf6 0.72s ease-out 0.38s both; }
      `}</style>
      <Navbar />
      <div className="pt-32 pb-16 px-6 max-w-lg mx-auto text-center">
        ...
        <h1 className="font-display text-3xl font-bold mb-3">{t("confirmation.title")}</h1>
        <p className="text-muted-foreground text-lg mb-8">{t("confirmation.subtitle")}</p>

        <div className="bg-white rounded-2xl p-6 mb-8 text-left" style={{ boxShadow: "var(--card-shadow)" }}>
          <div className="flex items-center gap-3 mb-4">
            <span className="text-2xl">👩‍🍳</span>
            <div className="flex-1">
              <p className="font-semibold mb-2">{t("confirmation.preparingTitle")}</p>
          ...
          <div className="flex items-center gap-3">
            <span className="text-2xl">📱</span>
            <div>
              <p className="font-semibold">{t("confirmation.trackTitle")}</p>
              <p className="text-sm text-muted-foreground">{t("confirmation.trackText")}</p>
            </div>
          </div>
        </div>

        <div className="flex flex-col sm:flex-row gap-3">
          <button onClick={() => navigate(lp("/mes-commandes"))}
